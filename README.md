...
它是一个 skill（`SKILL.md` + `references/` + `templates/` + `scripts/`），也是一个 DSH agent preset（`apical-bud`）的方法论内核。四个不常见的主张：

1. **推演，不是头脑风暴。** 每个概念节点都必须写清它从哪个父节点推出来，祖先链必须能走到种子。门禁会拒绝孤儿节点——凭空的灵感可以讨论，但不能冒充推演。
2. **分层也是一种解读。** 同一个需求至少有几种分法；只写一种，就等于用我的措辞替换了你的需求。所以每层必须保留「其他解读（被否）」，并由你确认。
3. **顶芽只有一个，剪掉的枝留着。** 收敛意味着选一个核心理念，并把竞争节点连同淘汰理由留在树上——半年后这是"为什么不选另一条路"的唯一答案。
4. **术语不能堆成墙。** 一轮最多引入一个新词，未确认的词同时最多一个，定义必须是人话且不能用未定义的词解释自己；**你可以改写任何问题与术语**，我负责全局替换并记下曾用名。
